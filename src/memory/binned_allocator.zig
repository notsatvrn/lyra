    ...
    return struct {
        ...
        fn alloc(ctx: *anyopaque, len: usize, alignment: std.mem.Alignment, ret_addr: usize) ?[*]u8 {
            ...
            if (self.backing_allocator.rawAlloc(len, alignment, ret_addr)) |ptr| {
                if (config.report_leaks) self.large_count.increment();
                return ptr;
            } else return null;
        }
        ...
        fn remap(ctx: *anyopaque, buf: []u8, alignment: std.mem.Alignment, new_len: usize, ret_addr: usize) ?[*]u8 {
            ...
            inline for (&self.bins) |*bin| {
                if (buf.len <= bin.size and align_ <= bin.size) {
                    // Check it still fits
                    if (new_len > prev_size and new_len <= bin.size) return buf.ptr;
                    // Doesn't fit, need to move to a new bin or a big allocation

                    const new = (if (new_len > largest)
                        self.backing_allocator.rawAlloc(new_len, alignment, ret_addr)
                    else
                        alloc(ctx, new_len, alignment, ret_addr)) orelse return null;

                    const lcl = @min(new_len, buf.len);
                    @memcpy(new[0..lcl], buf[0..lcl]);
        ...
        fn free(ctx: *anyopaque, buf: []u8, alignment: std.mem.Alignment, ret_addr: usize) void {
            ...
            if (config.report_leaks) self.large_count.decrement();
        }
